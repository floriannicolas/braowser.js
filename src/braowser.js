...
// Battle-tested v1 mobile regex, carried over verbatim from the legacy
// source. MOBILE_REGEX_A matches the full UA against ~50 mobile-device
// keywords; MOBILE_REGEX_B is matched against the first 4 chars only and
// catches old carrier-prefix UAs (a 2010-era convention).
//
// MOBILE_REGEX_A is reused below in DEVICE_PATTERNS as a fallback "this is
// some kind of mobile device" rule; the full `isMobileUA` (both halves)
// drives `isMobile()` and the `mobile` HTML class for v1 CSS back-compat.
...
// Per-vendor patterns first, then generic type-only fallbacks. `matchFirst`
// returns on first hit, so the order also encodes specificity.
const DEVICE_PATTERNS = [
    ...
    // Generic Android — `Mobile` token is the canonical phone signal; without it, treat as tablet.
    [/Android.*\bMobile\b/,                         ()   => ({ vendor: undefined, model: undefined,  type: 'mobile' })],
    [/Android/,                                     ()   => ({ vendor: undefined, model: undefined,  type: 'tablet' })],
    // Non-Android mobile devices (BlackBerry, old feature phones, …).
    [MOBILE_REGEX_A,                                ()   => ({ vendor: undefined, model: undefined,  type: 'mobile' })],
];
...
// Maps our internal browser names to the brand strings that
// `navigator.userAgentData.brands` / `fullVersionList` actually emit.
// Used by `getResultAsync()` to pick the right entry for version enrichment.
const BROWSER_TO_BRAND = {
...
// Find the UA-CH brand entry that matches our internal browser name.
function findBrandVersion(brands, browserName) {
    const wanted = BROWSER_TO_BRAND[browserName] ?? browserName.toLowerCase();
    return brands.find((b) => b.brand.toLowerCase() === wanted);
}
...
export class Braowser {
    ...
    /** @returns {IDevice} */
    getDevice() {
        const base = matchFirst(this._ua, DEVICE_PATTERNS)
            ?? { vendor: undefined, model: undefined, type: undefined };
        // iPadOS 13+ ships a desktop-Safari UA. The only sync signal that
        // tells us it's really an iPad is a runtime DOM probe, so this one
        // override lives outside the UA-only DEVICE_PATTERNS table.
        return (base.model === 'Macintosh' && looksLikeIpad())
    ...
    async getResultAsync() {
        ...
        // Browser: pick the brand that matches our detected name and use its
        // full version. Engine version follows when engine is Blink — Blink
        // tracks Chrome's full version.
        if (Array.isArray(hints.fullVersionList) && result.browser.name) {
            const brand = findBrandVersion(hints.fullVersionList, result.browser.name);
            if (brand) {
                result.browser = { ...result.browser, version: brand.version, major: majorOf(brand.version) };
